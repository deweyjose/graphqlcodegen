...
public class Codegen extends AbstractMojo {
	...
	private Set<File> getSchemaPaths() {
		...
		final Set<File> configuredSchemaPaths = stream(schemaPaths).collect(toSet());
		final Set<File> expandedSchemaPaths = new HashSet<>();

		// expand any directories into graphql file paths
		for (final File path : configuredSchemaPaths) {
			if (path.isFile()) {
	...
	public void execute() {
		if (!skip) {
			...
			final Set<File> schemaPaths = getSchemaPaths();

			final SchemaFileManifest manifest = new SchemaFileManifest(new File(outputDir, "schema-manifest.props"));
