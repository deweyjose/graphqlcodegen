...
public class Validations {
   ...
   *
   * <p>We maintain two sets: encounteredDirectories to keep track of encountered directories
   * encounteredPaths to keep track of encountered paths.
   *
   * <p>We iterate through the sorted files and perform the following checks: Directory overlap:
   * throw an exception if duplicate directories are configured. Duplicate files: throw an exception
   * if duplicate files are configured. Ancestry in already added directories: if files exist in
   * configured directories or subdirectories of configured directories throw an exception
   *
   * @param files
   */
    ...
    for (File file : sortedFiles) {
      ...
      while (parent != null) {
        if (encounteredDirectories.contains(parent)) {
          throw new IllegalArgumentException(
              String.format(
                  "Overlap configured: %s contains %s", parent.getPath(), file.getPath()));
        }
